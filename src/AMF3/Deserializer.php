...
class Deserializer extends \SabreAMF\Deserializer
{
    ...
    public function readAMFData($settype = null)
    {
        ...
        switch ($settype) {
            ...
            case Constants::DT_XMLSTRING:
                return $this->readXMLString();
            case Constants::DT_BYTEARRAY:
                return $this->readByteArray();
            default:
                throw new Exception('Unsupported type: 0x' . strtoupper(str_pad(dechex($settype), 2, 0, STR_PAD_LEFT)));
                return false;
        }
    }
        ...
        if ($storedObject) {
            ...
            if ($className) {
                ...
                if ($localClassName = $this->getLocalClassName($className)) {
                    ...
                    $rObject = new \SabreAMF\TypedObject($className, array());
                }
            } else {
            ...
            if ($encodingType & Constants::ET_EXTERNALIZED) {
                ...
                if (!$storedClass) {
                    $this->storedClasses[] = array(
                        'className' => $className,
                        'encodingType' => $encodingType,
                        'propertyNames' => $propertyNames
                    );
                }
                if ($rObject instanceof \SabreAMF\Externalized) {
                    $rObject->readExternal($this->readAMFData());
                } elseif ($rObject instanceof \SabreAMF\TypedObject) {
                ...
                if ($rObject instanceof \SabreAMF\TypedObject) {
    ...
    public function readByteArray()
    {
        ...
        $strlen = $strref >> 1;
        $str = $this->stream->readBuffer($strlen);
        return new \SabreAMF\ByteArray($str);
    }
    ...
    public function readInt()
    {
        ...
        if (($int & 0x18000000) == 0x18000000) {
            ...
            $int *= -1;
            $int -= 1;
        } elseif (($int & 0x10000000) == 0x10000000) {
