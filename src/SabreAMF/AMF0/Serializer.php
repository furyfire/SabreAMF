    ...
    public function writeAMFData($data, $forcetype = null)
    {
        ...
        if (is_null($forcetype)) {
            ...
            if (!$type && is_object($data)) {
                ...
                } elseif ($this->getRemoteClassName(get_class($data))) {
                    $type = Constants::DT_TYPEDOBJECT;
                    // Otherwise.. check if it its an TypedObject
                } elseif ($data instanceof \SabreAMF\ITypedObject) {
                    $type = Constants::DT_TYPEDOBJECT;
        ...
        switch ($type) {
            case Constants::DT_NUMBER:
                return $this->stream->writeDouble($data);
            case Constants::DT_BOOL:
                return $this->stream->writeByte($data == true);
            ...
            case Constants::DT_NULL:
                return true;
            case Constants::DT_MIXEDARRAY:
                return $this->writeMixedArray($data);
            case Constants::DT_ARRAY:
                return $this->writeArray($data);
            case Constants::DT_DATE:
                return $this->writeDate($data);
            case Constants::DT_LONGSTRING:
                return $this->writeLongString($data);
            case Constants::DT_TYPEDOBJECT:
                return $this->writeTypedObject($data);
            case Constants::DT_AMF3:
                return $this->writeAMF3Data($data);
            default:
