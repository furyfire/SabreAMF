 ...
 * InputStream 
 * 
 * This is the InputStream class. 
 * You construct it with binary data and it can read doubles, longs, 
 * ints, bytes, etc. while maintaining the cursor position
 * 
 * @package SabreAMF 
 * @version $Id$
...
class InputStream
{
    ...
    public function &readBuffer($length)
    {
        ...
        if ($length + $this->cursor > strlen($this->rawData)) {
            throw new Exception('Buffer underrun at: ' . $this->cursor . '. Trying to fetch ' . $length . ' bytes');
            return false;
        }
