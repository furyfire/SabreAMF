 ...
 * AMF Client
 *
 * Use this class to make a calls to AMF0/AMF3 services. 
 * The class makes use of the curl http library, so make sure you have this installed.
 *
 * It sends AMF0 encoded data by default. 
 * Change the encoding to AMF3 with setEncoding. 
 * sendRequest calls the actual service 
 * 
 * @package SabreAMF
 * @version $Id$
...
class Client
{
    ...
    public function __construct($endPoint)
    {
        ...
        $this->amfRequest = new Message();
        $this->amfOutputStream = new OutputStream();
    }
     ...
     * It expects the servicepath and methodname, and the parameters of the methodcall
     * 
    ...
    public function sendRequest($servicePath, $data)
    {
        ...
        if ($this->encoding & Constants::FLEXMSG) {
            ...
            $message = new AMF3\RemotingMessage();
            $message->body = $data;
        ...
        $this->amfRequest->addBody(
            ...
            'target' => $this->encoding & Constants::FLEXMSG ? 'null' : $servicePath,
            'response' => '/1',
            'data' => $data
            )
        );
        ...
        $headers = array_merge(
            array('Content-type: ' . Constants::MIMETYPE),
            $this->httpHeaders
        );
        ...
        $this->amfInputStream = new InputStream($result);
    ...
    public function setEncoding($encoding)
    {
        ...
        $this->amfRequest->setEncoding($encoding & Constants::AMF3);
    }
}
