 ...
 * Message 
 * 
 * The Message class encapsulates either an entire request package
 * or an entire result package; including an AMF enveloppe
 * 
 * @package SabreAMF 
 * @version $Id$
 ...
 * @uses AMF0_Serializer
 * @uses AMF0_Deserializer
 */
class Message
{
     ...
     * @var int 
     */
    private $encoding = Constants::AMF0;

    /**
     * serialize 
     * 
     * This method serializes a request. It requires an OutputStream as an argument to read
     * the AMF Data from. After serialization the Outputstream will contain the encoded AMF data.
     * 
     * @param OutputStream $stream 
     * @return void
     */
    public function serialize(OutputStream $stream)
    {
        ...
        foreach ($this->bodies as $body) {
            ...
            switch ($this->encoding) {
                case Constants::AMF0:
                    ...
                    break;
                case Constants::AMF3:
                    $serializer->writeAMFData(new AMF3\Wrapper($body['data']));
                    break;
            }
        }
    }
     ...
     * deserialize 
     * 
     * This method deserializes a request. It requires an InputStream with valid AMF data. After
     * deserialization the contents of the request can be found through the getBodies and getHeaders methods
     *
     * @param InputStream $stream 
     * @return void
     */
    public function deserialize(InputStream $stream)
    {
        ...
        $deserializer = new AMF0\Deserializer($stream);
        ...
        for ($i = 0; $i < $totalBodies; $i++) {
            ...
            if (is_object($body['data']) && $body['data'] instanceof AMF3\Wrapper) {
                $body['data'] = $body['data']->getData();
                $this->encoding = Constants::AMF3;
            } elseif (
                    is_array($body['data']) &&
                    isset($body['data'][0]) &&
                    is_object($body['data'][0]) &&
                    $body['data'][0] instanceof AMF3\Wrapper
            ) {
     ...
     * getClientType 
     * 
     * Returns the ClientType for the request. Check Constants for possible (known) values
     * 
     * @return int 
     */
